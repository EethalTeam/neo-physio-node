...
exports.createModalities = async (req, res) => {
    try {
        const { modalitiesCode, modalitiesName, isActive  } = req.body;
        ...
        if (existingModalities) {
            return res.status(400).json({ message: 'Modalities with this code or name already exists' });
        }
        // Create and save the new existingModalities
        const modalities = new Modalities({  modalitiesCode, modalitiesName, isActive  });
...
exports.getModalitiesByName = async (req, res) => {
    try {
        const  modalities = await Modalities.findOne({ modalitiesName: req.body.modalitiesName })
...
exports.updateModalities = async (req, res) => {
    try {
        const { _id, modalitiesCode, modalitiesName, isActive   } = req.body

        const modalities = await Modalities.findByIdAndUpdate(
            _id,
            { $set: { modalitiesCode, modalitiesName, isActive  } },
        ...
        res.status(200).json({ message: 'Modalities updated successfully', data:modalities  });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
};
